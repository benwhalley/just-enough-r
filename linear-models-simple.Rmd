...
Despite being tiny^[You might want to expand it in the RStudio interface, or save it to a pdf to print or blow-up on screen - at least at this exploratory stage.], the pairs plot suggests that `mpg` might be related to a number of the other variables including `disp` (engine size) and `wt` (car weight). We might want to take a closer look by selecting just those variables:
