...
`r margin_note("This same problem of 'too many ways to do it' applies to graphics in R too — perhaps even more so.")`
...
You can use a patterns to match a subset of the columns you want. For example, here we select all thecolumns where the name contains the letter `d`:
...
`r margin_note('There are other commands too, but these are probably the most useful to begin with. See the help files for more information.')`
...
<br><br><br>
  ...
  ### Think of data 'flowing' down the page
...
So we have achieved the same outcome, but the code reads as a series of operations which the data flows through, connected by our pipes (the `%>%`). At the end of the last pipe, our data gets dumped into the `summary()` function.
