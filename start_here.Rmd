...
Or if we want to be explicit^[R often makes sensible assumptions about what we want. Here the default assumption is that if we type the *name* of a variable we want to see the *value* associated with it.]:
...
We can check the values stored in these variables by printing them, e.g.:
...
Once we have created a vector, we sometimes want to access the individual elements from it again. We do this based on their *position*. 
...
We can display the whole vector by just typing it's name. But if we want to show only the *first* element of this vector, we type:
...
## Selecting more than one vector element
...
We can also make subsets of the original vector and assign them to a *new* variable:
...
There are lots of other neat tricks you can use when subsetting, but we don't need them all right now. If you want to know more see <http://www.statmethods.net/management/subset.html>
...
You might have noticed that each value in the result of the `quantile` function is printed with a label indicating which percentile it refers to. The `quantile` function actually returns a *named vector*, which can be helpful to make results more readable. If you wanted to do this in your own code see: <http://www.r-tutor.com/r-introduction/vector/named-vector-member>.
